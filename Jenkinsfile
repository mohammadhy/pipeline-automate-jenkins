pipeline {
    agent any
    environment {
      REGISTERY ='192.168.42.3:5000'
      IMAGE = "${env.JOB_NAME}"
      PRODUCT_ID = '1'
      DOJO_URL = '192.168.42.12:8080'
      ENGAGEMENT_ID = '2'
      PRODUCT_NAME = 'milad-255'
    }
    ...
    stages {
        stage('checkout') {
            steps {
              git branch: 'develop', credentialsId: '26e885c2-5e8a-44c9-8eff-95fb21ad957b', url: 'http://192.168.42.2:8929/255-Backend/api-gateway.git'
                }
            }
        stage('Get Short SHA'){
          steps {
        ...
        stage('Trivy'){ 
            steps{
                sh ''' trivy image --db-repository $REGISTERY/aquasecurity/trivy-db:2 --java-db-repository $REGISTERY/aquasecurity/trivy-java-db:1 -f json -o trivy.json $REGISTRY/$IMAGE:$CI_COMMIT_SHORT_SHA '''
            }
        }
